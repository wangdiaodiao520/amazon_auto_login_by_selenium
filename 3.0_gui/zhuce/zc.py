...
class Zc():
    def __init__(self,dl_api,t,yy_id,yy_sec,ym_token,display,*zc_list):
        self.dl_api =dl_api
        self.timeout = t
        self.yy_id = yy_id
        self.yy_sec = yy_sec
    ...
    def zc(self):
        try:
            ...
            wait_long = WebDriverWait(browser,self.timeout*10,1)
            ...
            while title_point:
                ...
                try:#--------打码平台----------#                  
                    #yzm = img_yz_t(img,self.yy_id,self.yy_sec)
                    yzm = img_yz(img)
                except:
                    raise YZ_error                
                browser.find_element_by_id('ap_password').send_keys(self.infor[1])
...
def start(dl_api,t,yy_id,yy_sec,ym_token,display,*zc_list):
    zc = Zc(dl_api,t,yy_id,yy_sec,ym_token,display,*zc_list)
    result = zc.zc()
    return result
