...
class Zc():
    def __init__(self,dl_api,t,yy_id,yy_sec,ym_token,display,nb,*zc_list):
        self.dl_api =dl_api
        self.timeout = t
        self.yy_id = yy_id
        self.yy_sec = yy_sec
    ...
    def zc(self):
        try:
            ...
            if len(self.dl_api) >= 10:
                ...
                profile.set_preference('network.proxy.http', ip(self.dl_api)[0])
                profile.set_preference('network.proxy.http_port', int(ip(self.dl_api)[1]))
                profile.update_preferences()
            else:
                pass
            browser = webdriver.Firefox(firefox_options=profile,executable_path='./geckodriver.exe')
            ...
            wait_long = WebDriverWait(browser,self.timeout*5,1)
            ...
            while title_point:
                ...
                try:#--------打码平台----------#
                    ...
                    raise YZ_error
                '''
                print self.yy_id,self.yy_sec
                yzm = img_yz_t(img,str(self.yy_id).strip(),str(self.yy_sec).strip())
                '''
...
def start(dl_api,t,yy_id,yy_sec,ym_token,display,c,*zc_list):
    zc = Zc(dl_api,t,yy_id,yy_sec,ym_token,display,c,*zc_list)
    result = zc.zc()
    return result
